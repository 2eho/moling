...
| **四库过滤** | 集合运算提取关联条目 | ✅ 已实现 | 100% | E5 已完成（2026-06-13） |
| **动态层冲突检测** | 状态机 vs 卡片方向 | ✅ 已实现 | 100% | E5 已完成（2026-06-13） |
| **方向冲突评分** | 实体+情感匹配 → 0-1分 | ✅ 已实现 | 100% | E5 已完成（2026-06-13） |
| **编织方案匹配** | 规则引擎选编织模式 | ✅ 已实现 | 100% | E5 已完成（2026-06-13） |
| **大纲模板填充** | 结构化数据填入模板 | ✅ 已实现 | 100% | E6 已完成（2026-06-13） |
| **LLM调用** | 3次调用（小/中/大模型） | ✅ 已实现 | 100% | E1-E3 已完成（2026-06-13） |
| **连贯性校验** | 规则引擎 vs 4库+动态层 | ✅ 已实现 | 100% | E10 已完成（2026-06-13） |
| **动态层更新** | 结构化操作（锚点/基线） | ✅ 已实现 | 100% | E11 已完成（2026-06-13） |
| **卡牌生命周期** | 新鲜期/成熟期/退役管理 | ✅ 已实现 | 100% | E7 已完成（2026-06-13） |
| **秘密矩阵** | 信息不对称追踪 | ✅ 已实现 | 100% | E11 已完成（2026-06-13） |
| **健康监控** | R1/R2/R3检测 | ✅ 已实现 | 100% | E11 已完成（2026-06-13） |
| **Phase 4 收纳** | 四库变更提取+合并 | ✅ 已实现 | 100% | E4 已完成（2026-06-13） |

### 1.3 实现深度评估

**已实现的功能**（在 `algorithm_service.py`/`card_service.py`/`health_service.py` 中）：
- ✅ 加权随机算法（按稀有度权重）+ 保底机制（E7）
- ✅ 新鲜期加权（freshness bonus）+ 淘汰检查逻辑（E7）
- ✅ 抽卡状态机（DrawStateMachine）（E8）
- ✅ 冲突检测（角色知识状态验证）（E5）
- ✅ 编织模式选择规则（E5）
- ✅ Prompt组装（4层注入）（E2）
- ✅ LLM集成（完整22步流水线）（E1-E3）
- ✅ 连贯性校验 + 动态层更新（E10-E11）
- ✅ 卡牌生命周期管理（E7）
- ✅ 健康监控规则（R1/R2/R3）（E11）

**无缺失的核心功能** — 2026-06-13 CHANGELOG 修复后核心算法已全部实现。

### 1.4 算法实现完成度总结

| 类别 | 完成度 | 说明 |
|------|--------|------|
| **基础抽卡** | 100% | E7-E8 已完成（加权随机、保底、新鲜期、淘汰） |
| **卡牌管理** | 100% | E7 已完成（生命周期管理） |
| **生成流水线** | 100% | E1-E3 已完成（22步完整流水线） |
| **Phase 4 收纳** | 100% | E4 已完成（LLM 提取 + 四库合并 + 健康检查） |
| **健康监控** | 100% | E11 已完成（R1/R2/R3 规则） |
| **整体完成度** | **95%** | 核心算法全部实现，仅剩边缘 case 待优化 |
...
| `/api/v1/projects/:pid/chapters/:id/redraw` | POST | 重抽 | ✅ 已实现 | 100% | C6 已完成（2026-06-13） |
| `/api/v1/projects/:pid/cards/draw-history` | GET | 抽卡历史 | ✅ 已实现 | 100% | E7 已完成（2026-06-13） |
...
| `/api/v1/subscriptions/create-checkout` | POST | 创建支付 | ✅ 已实现 | 100% | D3 已完成（2026-06-13） |
