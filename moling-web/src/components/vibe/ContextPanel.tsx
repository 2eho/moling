  ...
  return (
    ...
    >
      ...
      <div className="glass-card p-3">
        ...
        <p className="text-[11px]" style={{ color: "var(--th-accent-text)" }}>
          {project.genre}
        </p>
        ...
        {unresolved.length > 0 ? (
          unresolved.map((f) => (
            <div key={f.id} className="flex items-start gap-1.5 mb-1">
              <AlertCircle size={10} className="mt-0.5 shrink-0" style={{ color: "var(--th-warning)", opacity: 0.6 }} />
          ...
          <p className="text-[10px]" style={{ color: "var(--th-text-4)" }}>暂无待回收伏笔</p>
        )}
      </div>

      {/* 世界观 */}
      <div className="glass-card p-3">
        <span className="text-[11px] font-medium" style={{ color: "var(--th-text-2)" }}>世界观设定</span>
