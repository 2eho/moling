...
def test_07_get_project_detail():
    ...
    try:
        ...
        if resp.status_code == 200:
            data = resp.json()
            log("获取项目详情", True, f"Title: {data.get('title')}")
            return True
        else:
...
def run_all_tests():
    ...
    with open("C:/Users/Admin/Desktop/新建文件夹 (2)/test_report_v2.json", "w", encoding="utf-8") as f:
        json.dump(report, f, indent=2, ensure_ascii=False)
    
    print("\n📝 详细报告已保存到: test_report_v2.json")
