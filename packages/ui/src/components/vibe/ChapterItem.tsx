 ...
 */
export function ChapterItem({ chapter, isActive, isDisabled, onClick }: ChapterItemProps) {
  const _isCompleted = chapter.status === "completed";
  ...
  return (
    ...
    >
      {/* Active pill */}
      {isActive && (
        <div className="absolute left-0.5 top-1.5 bottom-1.5 w-[5px] z-10 bg-th-accent-text rounded-full" />
      )}
      {/* 5px spacer */}
      <span className="inline-block w-[5px] shrink-0 overflow-hidden">&nbsp;</span>
      {/* 标题 — 左对齐，主信息 */}
      <span className="truncate text-[13px] py-2.5 md:py-2 leading-snug">{chapter.title}</span>

      {/* 编号 — 右对齐，样式与标题同步 */}
      <span className="text-[13px] py-2.5 md:py-2 leading-snug shrink-0 font-semibold ml-auto">
        {chapter.id}
      </span>
      {/* 右侧间距 */}
      <span className="inline-block w-[5px] shrink-0 overflow-hidden">&nbsp;</span>
