...
> **版本**: 2.8.0 | **最后更新**: 2026-06-24
> 本文档整合了 P0/P1 规格、卡牌组合算法规格及架构加固实现规格。**Rust 重写已为主线**，Python 遗留代码处于维护模式。
...
**目标**: 替代散落的异常抛出，建立机器可读、人类可理解、调用方可捕获的错误体系。

**Python 实现** (`app/errors.py` — 遗留):

| 组件 | 说明 |
|------|------|
| `ErrorCode` (IntEnum) | 数字编码 = `HTTP状态码 × 100 + 序号` |
| `AppError` | 统一基类，接收 `ErrorCode` + 可选 `detail` |
| 子异常类 | `NotFoundError`, `AuthError`, `PermissionError`, 等 |

**Rust 实现** (`moling-core/src/error.rs` — 主线):

| 组件 | 说明 |
|------|------|
| `AppError` enum | 编译时穷尽的错误变体，`#[derive(thiserror::Error)]` |
| `impl IntoResponse for AppError` | 自动映射为 HTTP 状态码 + JSON 响应体 |
| Tower 中间件 | 全局错误捕获，统一格式 `{"code": 40101, "message": "..."}` |

### 4.2 Worker 数据库会话管理

**目标**: 统一 Worker 的数据库访问模式。

**Python 实现** (`app/worker/db.py` — 遗留):

| 组件 | 说明 |
|------|------|
| `get_worker_session()` | 惰性引擎创建，pool_size=2, pool_pre_ping=True |
| `dispose_worker_engine()` | Celery `worker_shutdown` 信号触发释放 |

**Rust 实现** (`moling-worker/src/db.rs` — 主线):

| 组件 | 说明 |
|------|------|
| `SeaORM DatabaseConnection` | 共享连接池（通过 AppState 注入） |
| `bb8-redis Pool` | 异步 Redis 连接池，多 Worker 共享 |
| 优雅关闭 | `tokio::signal` + `CancellationToken` |
...
**Rust 模式** (主线):
```
Tokio timeout (软超时) → 重新投递任务
    ↓
可重试异常 (ConnectionError, TimeoutError, DBError) → 指数退避重试
    ↓
通用 Error → Tracing 日志，标记 FAILED + 死信队列
```
...
**Python 实现** (`app/worker/celery_app.py` → `beat_schedule` — 遗留):

| 任务 | 调度周期 | 用途 |
|------|---------|------|
| `phase4-auto-advance` | 每小时 | 扫描自动审核项目，触发 Phase 4 |
| `vault-periodic-reanalyze` | 每 6 小时 | Vault 重分析 |
| `card-retire-check` | 每天 | 卡片池过期检查 |
| `health-auto-notify` | 每 30 分钟 | 健康检查 + 告警 |

**Rust 实现** (`moling-worker/src/cron.rs` — 主线):
- Tokio `interval` + `tokio::spawn` 替代 Celery Beat
- 启动命令: `cargo run --bin moling-worker`

### 4.5 健康检查端点

**目标**: `GET /api/v1/health` 验证 DB + Redis + Worker 三方连通性。

| 检查项 | 方法 | 超时 | 失败行为 |
|--------|------|------|----------|
| Database | `SELECT 1` (SeaORM) | 继承 pool 超时 | status="degraded" |
| Redis | `PING` | 3s | status="degraded" |
| Worker | 内部健康报告 | 3s | status="degraded" |
...
**目标**: 所有数据访问对象共享统一的查询行为和安全约束。

| 规范 | Rust 实现 (moling-db) | Python 遗留 |
|------|----------------------|------------|
| limit 钳制 | 编译时常量 `DEFAULT_MAX_LIMIT=500` | 同左 |
| 软删除约定 | `include_deleted: bool = false` 参数默认过滤 | 同左 |
| 游标分页 | `list_cursor() -> (Vec<Model>, Option<Cursor>)` | `list_cursor()` 返回 `(items, next_cursor)` |
| 事务契约 | DAO 接收 `&DatabaseConnection`，**禁止内部 commit** | 同左 |

### 4.8 安全加固

| 加固项 | Rust 实现 | 说明 |
|--------|----------|------|
| 请求体限制 | Tower-HTTP `ContentLengthLimit` (10MB) | 中间件层前置拦截 |
| Redis 密码 | `REDIS_PASSWORD` env var | 生产环境警告 |
| CORS 审查 | Tower-HTTP CORS 中间件 | 生产环境禁用 `*` |
| Refresh Token 轮换 | Redis 黑名单 (TTL = 过期时间) | 签发新 token 同时作废旧 token |
| SQL 注入防护 | SeaORM 参数化查询 | 编译时安全，零注入风险 |

### 4.9 跨平台兼容

> **Rust 原生跨平台**: Rust 编译为原生二进制，Windows/macOS/Linux 均可直接运行，无需 greenlet 补丁或平台适配层。
...
> **注意**: 以上 Python 平台适配已随 Python → Rust 迁移废弃。参见 [ADR-001](./adr/adr-001-python-to-rust.md)。
...
# 4. 单元测试
cargo test --workspace       # 274/274 passed, 0 failed
...
| 版本 | 日期 | 内容 |
|:----|:----|:-----|
| 2.8.0 | 2026-06-24 | Rust 主线对齐：§4.1 统一错误体系(Rust AppError enum + thiserror)、§4.2 Worker DB(SeaORM 连接池)、§4.3 Worker 异常(Tokio timeout)、§4.4 定时调度(Rust Cron)、§4.5 健康检查、§4.7 DAO(Rust trait)、§4.8 安全加固(Tower-HTTP)、§4.9 跨平台(Rust 原生)、§5 质量门禁(Cargo + Vite) | Moling Team |
