...
### 4.18 LLM 集成深度扫描 v4 发现（2026-06-21）
...
| ID | 严重度 | 问题 | 位置 | 影响 |
|----|:--:|------|------|------|
| **L1** | CRITICAL | Token 预算绕过：`_chat_stream` 流式请求不调用 `budget_manager.record_usage()` | `client.py:579` | ✅ 修复：流式路径补全记录 |
| **L2** | CRITICAL | `ContextBudget` 完整实现但 LLMClient 从未调用 | `context_budget.py` | Prompt 可能超上下文窗口 |
| **L3** | HIGH | KeyManager `_recover_key` 后 backoff_level 不重置 | `key_manager.py` | ✅ 修复：恢复时 `backoff_level=0` |
| **L4** | HIGH | `get_effective_llm_config()` 硬编码 default，不读 `LLM_MODEL` 配置 | `config.py:263` | ✅ 修复：读取 `s.LLM_MODEL` |
