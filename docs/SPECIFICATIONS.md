...
| ID | 问题 | 位置 | 影响 |
|----|------|------|------|
| **P2-2** | Redis `release_lock()` 无 owner 验证，直接 DELETE key | `phase4_store.py:107` | 可能误删其他 Worker 持有的锁 |
| **P9-2** | R2 健康规则依赖不存在的 `event_type` 字段 | `health_monitor.py` + 模型 | R2 告警永不被触发 |
| **P5-3** | `consecutive_failures` 计数器全局共享 | `phase4_scheduler.py` | 跨项目错误告警 |
| **P6-4** | Stale 检查只看 `planted_chapter` 不看 `advancement_log` | `merge_service.py:772` | 误标 stale 承诺 |
...
| ID | 问题 | 位置 | 影响 |
|----|------|------|------|
| **C1** | greenlet 补丁块引用未定义 `logger` | `dependencies.py:41,65` | Windows 启动 NameError 崩溃 |
...
| ID | 问题 | 位置 | 严重度 | 影响 |
|----|------|------|:--:|------|
| **S1** | Access Token 硬编码 `timedelta(minutes=15)` 忽视 `ACCESS_TOKEN_EXPIRE_MINUTES` | `auth_service.py:52` | P0 | 配置修改不生效 |
| **S2** | Refresh Token 硬编码 `timedelta(days=30)` 忽视 `REFRESH_TOKEN_EXPIRE_DAYS=7` | `auth_service.py:67` | P0 | 实际有效期是配置的 4 倍 |
